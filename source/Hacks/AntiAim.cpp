...
{
    if (ImGui::MenuItem("spinbot")) {
        antiAimOpen = true;
        ImGui::SetWindowFocus("aaaaaaaaa");
        ImGui::SetWindowPos("aaaaaaaaaaaa", { 100.0f, 100.0f });
    }
} 

void AntiAim::tabItem() noexcept
{
    if (ImGui::BeginTabItem("spinbot")) {
        drawGUI(true);
        ImGui::EndTabItem();
    }
} 
...
void AntiAim::drawGUI(bool contentOnly) noexcept
{
    if (!contentOnly) {
        ...
        ImGui::Begin("aaaaaaaaaaaaa", &antiAimOpen, ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse);
    }
